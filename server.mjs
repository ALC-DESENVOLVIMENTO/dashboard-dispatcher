...
function mutationAllowed(req) { if (!isProduction) return process.env.ADMIN_MUTATIONS_ENABLED !== 'false'; if (process.env.ADMIN_MUTATIONS_ENABLED !== 'true') return false; const allowlist = String(process.env.ADMIN_IP_ALLOWLIST || '').split(',').map(value => value.trim()).filter(Boolean); return allowlist.includes(clientIp(req)); }
function privateAdminReadAllowed(req) { if (!isProduction) return true; if (process.env.ADMIN_READ_ENABLED !== 'true') return false; const allowlist = String(process.env.ADMIN_IP_ALLOWLIST || '').split(',').map(value => value.trim()).filter(Boolean); return allowlist.includes(clientIp(req)); }
