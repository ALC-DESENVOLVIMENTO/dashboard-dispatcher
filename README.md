...
2. A linha entra no universo de Frota Fixa quando `modalidade` é `FROTA FIXA` ou `reserva_ff` é verdadeiro.
3. Linhas de ambulância são desconsideradas quando `Cluster = ROTA`, mesmo que estejam marcadas como Frota Fixa ou Reserva FF.
4. O portão de DS é `DS médio por rota >= 92%`.
5. A utilização mensal é calculada como `rotas_utilizadas / rotas_FF_elegíveis`.
6. A maior faixa atingida define o pagamento: 100% = R$ 1.000; 95% = R$ 700; 90% = R$ 350. Abaixo de 90% ou sem passar no DS = R$ 0.
